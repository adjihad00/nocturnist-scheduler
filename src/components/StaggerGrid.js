...
import calc, { buildCycleMap } from '../utils/calc';
...
export default function StaggerGrid({ pattern }) {
  ...
  for (let g = 0; g < Math.min(actualGroups, 8); g++) {
    ...
    for (let d = 0; d < daysToShow; d++) {
      ...
      const status = cycleMap[posInCycle];
      const isOn = status === 'on';
      const isMini = status === 'mini-break';
      cells.push(
        <div key={d} style={{
          width: 16, height: 16, borderRadius: 2,
          background: isOn ? '#1b6ca8' : isMini ? '#3d3520' : '#1e1e30',
          border: isOn ? '1px solid #3ba4e088' : isMini ? '1px dashed #7a6a30' : '1px solid #2a2a40',
          transition: 'all 0.2s',
        }} />,
      );
    }
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 6, fontSize: 10, color: '#6666aa' }}>
        ...
        {pattern.type === 'split' && (
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <div style={{ width: 10, height: 10, background: '#3d3520', border: '1px dashed #7a6a30', borderRadius: 2 }} /> Mini-break
          </span>
        )}
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
