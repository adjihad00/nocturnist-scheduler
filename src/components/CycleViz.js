export default function CycleViz({ pattern }) {
  const cycle = pattern.on + pattern.off;
  const days = [];
  for (let i = 0; i < cycle; i++) {
    days.push(i < pattern.on);
  }
  return (
    <div style={{ display: 'flex', gap: 3, alignItems: 'center', marginTop: 8 }}>
      {days.map((isOn, i) => (
        <div key={i} style={{
          width: 28, height: 28, borderRadius: 4,
          background: isOn
            ? 'linear-gradient(135deg, #0f4c75, #1b6ca8)'
            : 'linear-gradient(135deg, #2a2a3e, #1a1a2e)',
          border: isOn ? '2px solid #3ba4e0' : '2px solid #333350',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 10, color: isOn ? '#a0d8ff' : '#555570', fontWeight: 600,
        }}>
          {i + 1}
        </div>
      ))}
      <span style={{ marginLeft: 8, fontSize: 12, color: '#8888aa', fontStyle: 'italic' }}>
        {pattern.on}N &rarr; {pattern.off} off &rarr; repeat
      </span>
    </div>
  );
}
